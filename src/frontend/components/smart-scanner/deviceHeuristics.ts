...
export function detectDeviceAndReadings(ocrInput: OcrResult | string): ParsedReading | null {
  ...
  for (const kw of keywords) {
    ...
    if (t.includes("PULSE") || t.includes("PUL/MIN") || t.includes("/MIN") || t.includes("BPM"))
      bpScore += 40;

    // Pulse Oximeter
    if (t.includes("SPO2") || t.includes("SP02") || t.includes("%SPO2") || t.includes("%O2"))
      oxScore += 100;
    if (t === "%") oxScore += 50;
    ...
    if (
      ...
      t.includes("CELSIUS") ||
      t.includes("FAHRENHEIT")
    )
      thScore += 50;

    // Weight Scale
    if (t.includes("KG") || t.includes("KILO") || t.includes("LBS") || t.includes("LB"))
      wtScore += 100;
  ...
  if (candidates.length >= 2) {
    ...
    if (bestSys && bestDia) {
      ...
      const pulseCandidates = candidates.filter(
        (c) => c.block !== sysBlock && c.block !== diaBlock && c.value >= 40 && c.value <= 180,
      );
      ...
      if (
        Math.abs(
          bestSys.block.x + bestSys.block.width / 2 - (bestDia.block.x + bestDia.block.width / 2),
        ) < 100
      )
        conf += 0.2; // good layout
      if (bestSys.value >= 90 && bestSys.value <= 140 && bestDia.value >= 50 && bestDia.value <= 90)
        conf += 0.15; // standard physiological range
  ...
  if (candidates.length >= 2) {
    ...
    if (bestSpo2 && bestPls) {
      ...
      if (bestSpo2.value >= 94) conf += 0.2; // physiological check bonus
  ...
  if (tempCandidates.length > 0) {
    ...
    if (bestCand.value > 50) unit = "°F";
      ...
      cleanedText.includes("°F") ||
      cleanedText.includes(" F ") ||
      cleanedText.endsWith(" F")
    )
      unit = "°F";

    const distToUnit = getMinDistanceToKeyword(bestCand.block, [
      "°C",
      "°F",
      "TEMP",
      "CELSIUS",
  ...
  const weightCandidates = candidates
    .map((c) => {
      let val = c.value;
      let recoveredDecimal = false;
      if (val >= 300 && val <= 2500) {
  ...
  if (weightCandidates.length > 0) {
    ...
    if (distToUnit < 250) conf += 0.2;
    if (bestCand.value >= 40 && bestCand.value <= 150 && unit === "kg") conf += 0.2; // normal range
  ...
  if (bestMatch.deviceType === "Blood Glucose Meter" && glScore > 100)
    bestMatch.confidence = Math.max(bestMatch.confidence, 0.85);
  if (bestMatch.deviceType === "Blood Pressure Monitor" && bpScore > 100)
    bestMatch.confidence = Math.max(bestMatch.confidence, 0.85);
  if (bestMatch.deviceType === "Pulse Oximeter" && oxScore > 100)
    bestMatch.confidence = Math.max(bestMatch.confidence, 0.85);
  if (bestMatch.deviceType === "Thermometer" && thScore > 100)
    bestMatch.confidence = Math.max(bestMatch.confidence, 0.85);
  if (bestMatch.deviceType === "Weight Scale" && wtScore > 100)
    bestMatch.confidence = Math.max(bestMatch.confidence, 0.85);
  ...
  switch (deviceType) {
    case "Blood Glucose Meter":
      ...
      if (data.glucose < lowLimit || data.glucose > highLimit) {
        return {
          valid: false,
          warning: `Glucose reading (${data.glucose} ${data.unit}) is in a critical range. Please confirm.`,
        };
      }
      break;
    case "Blood Pressure Monitor":
      if (
        ...
        return {
          valid: false,
          warning: `Blood Pressure (${data.systolic}/${data.diastolic} mmHg) is outside normal ranges. Please verify.`,
        };
      }
      break;
    case "Pulse Oximeter":
      if (data.spo2 < 85) {
        return {
          valid: false,
          warning: `SpO2 reading (${data.spo2}%) is abnormally low. Please verify.`,
        };
      }
      break;
    case "Thermometer":
      ...
      if (tempC < 34.5 || tempC > 41.5) {
        return {
          valid: false,
          warning: `Body temperature (${data.temperature}${data.unit}) seems unusual. Please verify.`,
        };
      }
      break;
    case "Weight Scale":
      ...
      if (wtKg < 35 || wtKg > 200) {
        return {
          valid: false,
          warning: `Weight (${data.weight} ${data.unit}) is unusual. Please confirm.`,
        };
      }
      break;
  }
